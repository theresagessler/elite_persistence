  ...
  dashboardBody(
    fluidRow(
      box(
        ...
        p("The app allows you to explore connections between Members of Parliament and slave owners at various degrees of separation. Graphs with many connections may load slower. Counts of connected slave-owners may include the target person themselves.")
...
server <- function(input, output, session) {
  ...
  results <- eventReactive(input$run, {
    ...
    g <- network_list[[paste0("degree_", input$degree)]]
    
    if (!(target_string %in% V(g)$name)) {
      return(list(error = TRUE, message = "No connections found."))
    }
